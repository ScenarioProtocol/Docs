...
export const description =
  'How the mechanisms for scenario pool creation and resolution, covering the movements of ETH within these steps and the roles played by protocol participants across both user types.'

# Protocol Tokenomics

The process shown in the [Workflow](/workflow) section explains the mechanisms for scenario pool creation and resolution, covering the movements of ETH within these steps and the roles played by protocol participants across both user types.
...
In order to support a safe, staged protocol roll-out (during which the V1 scenario pool contracts will be used to test and monitor user behaviour and incentive balancing), the SNR token will not be generated at protocol launch (see Section XXX: Staged roll-out).

## Token's Primary functions

### 1 - Revenue Distribution

6% of all funds in each scenario pool, once resolved, will be returned to ‘the protocol’ as revenue. 

Of this proportion, 4% will be sent as revenue to SNR stakers and 2% will be sent to the protocol’s growth pool (see section XXX). 

SNR will be staked and/or used to provide liquidity, with these actions giving users a ‘staking weight’ which will then determine their proportional claim on the 4% of all protocol revenues directed to stakers.
...
At scale, SCENARIO will be populated with many thousands of scenario pools, and thousands of active confirmers. Competitor products would first need to compete on factors such as network size, overall bounty pool size and speed of scenario confirmation, before being able to compete by bettering SCENARIO’s 6% take-rate. 
...
(See section XXX: Protocol revenue) 
...
1. The planned pay-out to that pool’s confirmers will be cancelled
2. The pool’s confirmer-maker will have 10% of their staked SNR slashed**
3. The pool’s confirmer-joiners will have 5% of their staked SNR slashed**
4. The pool will re-open to additional asker-joiners or for fund removal
5. Slashed SNR will be sent to the builder bounty fund (50%) and the growth pool (50%)
...
The protocol’s early advisors will play a key role in assisting to refine the protocol’s planned tokenomics and refining the balance between incentives and penalties.

It is possible that a final additional layer of arbitration may need to be added (as seen in UMA protocol). If this is deemed necessary, the SNR token will play a crucial role in this arbitration process.
